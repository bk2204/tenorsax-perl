...
foreach my $aa ($aas, $aaa) {
	...
	like(run("$ee.EE A\n"), qr/A\./ms, "de - arguments are interpolated");
	like(run("$ee.EE A B\n"), qr/A\..*B\./ms,
		"de - multiple arguments are interpolated");
	like(run("$ff.FF A\n"), qr/FF\./ms, "de - argument 0 works");
	...
	is(run("$aa$bb.rm BB\n.AA\n"),
		run("$aa.AA"), "rm - removed macro treated like nonexistent macro");
	...
	is(run(".cp 0\n$bb.als XX BB\n.rn BB\n.XX\n"),
		'BB.', "als - alias works even after original is removed");
}
