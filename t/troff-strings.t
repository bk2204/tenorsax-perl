...
foreach my $test (@tests) {
	...
	foreach my $x (qw/call eval/) {
		...
		is(run("$cp.ds $name text\n$eval\n"),
			"text", "ds - $name ($type) - can define and $x a string");
		is(run(qq{$cp.ds $name "text"\n$eval\n}),
			'text"', "ds - $name ($type) - handles final quote okay in $x");
		is(
			run(qq{$cp.ds $name "Have a nice day!\n$eval\n}),
			'Have a nice day!',
		...
		if ($cpval == 0) {
			is(run(".do ds $name text\n.cp 0\n$eval\n"),
				"text", "ds - $name (2) - can define and $x a string");
			is(run(qq{.do ds $name "text"\n.cp 0\n$eval\n}),
				'text"', "ds - $name (2) - handles final quote okay in $x");
			is(
				run(qq{.do ds $name "Have a nice day!\n.cp 0\n$eval\n}),
				'Have a nice day!',
